...
import('@dimforge/rapier3d').then((RAPIER) => {
  ...
  let heightMesh = undefined
  // https://github.com/viridia/demo-rapier-three/blob/main/src/terrain/TerrainShape.ts#L108
  // check out :arrow_up:
  const createHeightMap = () => {
    ...
    const planeBufferGeometry = new THREE.PlaneGeometry(width, height, width - 1, height - 1)
    ...
    heightMesh = new THREE.Mesh(
      planeBufferGeometry,
      new THREE.MeshStandardMaterial({ side: DoubleSide, color: 0xff3333, flatShading: false })
    )
    heightMesh.translateY(0)

    const rapierArray = new Float32Array(width * height)
    for (let x = 0; x < width; x++) {
      for (let y = 0; y < height; y++) {
        rapierArray[y * width + x] = floats[(x * height + y) * 3 + 2]
      }
    }

    console.log(points)
    console.log(rapierArray)
    const rigidBodyDesc = new RAPIER.RigidBodyDesc(RigidBodyType.Fixed)
      .setTranslation(0.0, 0, 0)
      .setCanSleep(false)
      .setCcdEnabled(false)
    const rigidBody = world.createRigidBody(rigidBodyDesc)
    const rigidBodyColliderDesc = RAPIER.ColliderDesc.heightfield(width - 1, height - 1, rapierArray, {
      x: width,
      y: 1,
      z: height
    })
    world.createCollider(rigidBodyColliderDesc, rigidBody)
  ...
  bootstrapMeshScene({
    ...
    addControls: (camera, renderer, scene, gui) => {
      ...
      const heightMeshFolder = gui.addFolder('Height Mesh settings')
      heightMeshFolder.add(heightMesh.rotation, 'x', -4, 4, 0.1)
      heightMeshFolder.add(heightMesh.rotation, 'y', -4, 4, 0.1)
      heightMeshFolder.add(heightMesh.rotation, 'z', -4, 4, 0.1)
